...
static void doBreadthFirstSearch(Config* conf)
{
	...
	bool sol = false; //If a solution was found the program should stop
	...
	while (length > 0 && !sol) {
		...
		#pragma omp parallel for private(lastBox) schedule(static, 1)
		for (uint64_t i = 0; i < length; i++) {
			...
			for (uint64_t b = 0; b < nBoxes; b++) {
				...
				for (uint64_t dir = 0; dir < 4; dir++) {
					...
					if ((c != Config::NONE)) {
						...
						#pragma omp critical
						newConfiguration = queue->lookup_and_add(c, i, newBox);
	...
	DFSStack* stack, DFSDepthMap* map)
{
	...
	if (Config::isSolutionConf(c)) {
		#pragma omp critical
		if(path == NULL || depth < path_len) //To prevent that a smaller solution will be overwritten
		{
			delete[] path;
	...
	for (uint64_t b = 0; b < nBoxes; b++) {
		...
		for (uint64_t dir = 0; dir < 4; dir++) {
			...
			if ((c != Config::NONE)) {
				...
				#pragma omp critical
				newConfig = map->lookup_and_set(c, depth + 1);
				if(newConfig) {
					// Recursively continue the search on a copy of the configuration
					DFSStack *stackCopy = new DFSStack(*stack);
					#pragma omp task shared(map) firstprivate(newBox, c, stackCopy)
					{
						Config next(c);
						recDepthFirstSearch(&next, newBox, stackCopy, map);
						delete stackCopy;
					}
				}
			}
