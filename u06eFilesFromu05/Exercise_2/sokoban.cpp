...
#pragma omp critical
	...
	for (uint64_t b = 0; b < nBoxes; b++)
	{
		...
		{
			uint64_t newBox;
			uint64_t nextConf = conf->getNextConfig(box, dir, &newBox);

			if ((nextConf != Config::NONE) && map->lookup_and_set(nextConf, depth + 1))
			{
				Config *next = new Config(nextConf);
				DFSStack *newStack = new DFSStack(*stack);
#pragma omp task firstprivate(next, newStack, newBox, map)
				{
					recDepthFirstSearch(next, newBox, newStack, map);
					delete next;
					delete newStack;
				}
			}
