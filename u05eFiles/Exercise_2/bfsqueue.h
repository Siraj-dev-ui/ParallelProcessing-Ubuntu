...
class BFSQueue
{
 private:
	...
	public:
		...
		inline void set(uint64_t aconfig, uint32_t apred, uint8_t abox) volatile {
			config = aconfig;
			pred = apred;
			box = abox;
		}
	...
	// The bit set is implemented as a two-level array of 64-bit values. For a given configuration
	// number, the function bsBitPos returns the bit position within an array element.
	
	static const uint64_t WORDBITS = 6;                 // 6 Bit = 0..63, bits in one uint64
	static const uint64_t WORDMASK = ((1<<WORDBITS)-1); // Bit mask where the last 6 Bits
	                                                        // are set
