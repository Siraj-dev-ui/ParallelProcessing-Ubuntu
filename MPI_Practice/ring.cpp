...
** COMPILE:     mpic++ -O -o ring ring.cpp
** RUN:         mpiexec -n 2 -machinefile machines $PWD/ring <msg-len>
**
...
int main (int argc, char **argv)
{
	...
	if (argc < 2) {
		if (myrank == 0) {
			cerr <<	"Usage: mpiexec -n <np> ... ring <msg-len>\n"
	...
	MPI_Send(buf, size, MPI_CHAR, (myrank+1) % nprocs,
			 0, MPI_COMM_WORLD);

	cout << "Process " << myrank << ": receiving ...\n" << flush;
	MPI_Recv(buf, size, MPI_CHAR, (myrank+nprocs-1) % nprocs,
