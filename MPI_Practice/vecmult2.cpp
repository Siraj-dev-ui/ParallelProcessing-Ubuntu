...
** COMPILE:     mpic++ -o vecmult2 vecmult2.cpp
** RUN:         mpiexec -n 2 -machinefile machines $PWD/vecmult2
**
** In this version the collective operations have been replaced by
...
int main (int argc, char **argv)
{
	...
	if (myrank == 0) {
		for (i=1; i<nprocs; i++) {
			MPI_Send(&factor, 1, MPI_DOUBLE, i, 0, MPI_COMM_WORLD);
		}
	}
	else {
		MPI_Recv(&factor, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status);
	}
	...
	if (myrank == 0) {
		for (i=1; i<nprocs; i++) {
			MPI_Send(&a[i*LOCAL_N], LOCAL_N, MPI_DOUBLE, i, 1, MPI_COMM_WORLD);
		}
		for (i=0; i<LOCAL_N; i++)
			local_a[i] = a[i];
	}
	else {
		MPI_Recv(local_a, LOCAL_N, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD, &status);
	}
	...
	//		     MPI_DOUBLE, 0, MPI_COMM_WORLD);
	if (myrank != 0) {
		MPI_Send(local_a, LOCAL_N, MPI_DOUBLE, 0, 2, MPI_COMM_WORLD);
	}
	else {
		for (i=1; i<nprocs; i++) {
			MPI_Recv(&a[i*LOCAL_N], LOCAL_N, MPI_DOUBLE, i, 2, MPI_COMM_WORLD,
					 &status);
		}
		for (i=0; i<LOCAL_N; i++)
			a[i] = local_a[i];
